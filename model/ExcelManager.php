...
class ExcelManager extends DatabaseManager {
	...
	*/
	...
	protected $addNumOfLinesValidation;
    ...
    function __construct($tableName="") {
		...
		$this->addNumOfLinesValidation = 0;
    }
	function Initialize(){
	...
	function PrepareExportingData(&$objPHPExcel, $currentTableName){
		...
		foreach($tableObject->_ as $headerColumn => $defaultValue){
			...
			if($this->addNumOfLinesValidation > 0){
				$numOfLinesValidation = $this->addNumOfLinesValidation;
			...
			switch ($tempDataType) {
				case $tempDataType==="date":
					$objPHPExcel->getActiveSheet()->getStyle($columnIndexInString.($rowCount+1).":".$columnIndexInString . $numOfLinesValidation)
					->getNumberFormat()
